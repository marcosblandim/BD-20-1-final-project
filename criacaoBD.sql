...
CREATE TABLE PACIENTE(
    ...
    DATA_INICIO DATE,
    DATA_FIM 	DATE,
    ID_HOSPITAL INTEGER UNSIGNED REFERENCES LEITO(ID_HOSPITAL),
    NUMERO INTEGER UNSIGNED REFERENCES LEITO(NUMERO)
    );
...
CREATE VIEW VisitantesDoLeito AS
SELECT VISITANTE.NOME AS 'VISITANTE', PACIENTE.ID_HOSPITAL AS 'ID_HOSPITAL', PACIENTE.NUMERO  AS 'LEITO', count(VISITAS.DATA_VISITA) AS 'NUMERO DE VISITAS'
FROM VISITANTE, VISITAS, PACIENTE
WHERE VISITAS.CPF_PACIENTE = PACIENTE.CPF
AND VISITAS.CPF_VISITANTE = VISITANTE.CPF
GROUP BY VISITANTE.NOME, PACIENTE.ID_HOSPITAL, PACIENTE.NUMERO;

DELIMITER $$
CREATE PROCEDURE RegistrarPaciente(IN CPF varchar(20),IN NOME varchar(65),IN SAUDE INT, IN DATA_NASC DATE, IN DATA_INICIO DATE, IN DATA_FIM DATE, IN HOSPITAL INT, IN LEITO INT)
BEGIN
INSERT INTO PACIENTE VALUES (CPF, NOME, SAUDE, DATA_NASC, DATA_INICIO, DATA_FIM, HOSPITAL, LEITO);
IF (DATA_FIM IS NULL)
THEN
	UPDATE LEITO
    SET INTERNADOS = INTERNADOS + 1
    WHERE ID_HOSPITAL = HOSPITAL AND NUMERO = LEITO;
END IF;
END
$$ DELIMITER ;
